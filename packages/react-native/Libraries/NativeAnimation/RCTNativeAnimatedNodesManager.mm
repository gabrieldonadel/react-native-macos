...
- (void)startAnimationLoopIfNeeded
{
  if ((_displayLink == nullptr) && _activeAnimations.count > 0) {
    _displayLink = [RCTPlatformDisplayLink displayLinkWithTarget:self selector:@selector(stepAnimations:)];  // [macOS] RCTPlatformDisplayLink; an alias for CADisplayLink off macOS
